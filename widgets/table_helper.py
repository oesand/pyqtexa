...
from .create.table_widget import table as table_widget, TableKwargs
from .create import tableRow, button as button_widget
...
class TableHelper(Generic[T]):
    ...
    def __init__(
        ...
        actions: dict[str, Callable[['TableHelper', T], None] | str] = None,
        ...
        **kwargs: Unpack[TableKwargs]
        ...
        self.__widget = table_widget(columns, **kwargs)
        if initial:
            self.extend(initial)
    ...
    def add(self, obj: T):
        ...
        if self.__actions:
            for name, handler in self.__actions.items():
                cells.append(button_widget(text=name, handle=lambda: handler(self, obj)))
